...
- [x] 截图工具栏（可选）：选区确认后显示「发送 / 保存 / 取消」浮动按钮
- [x] 单元测试：裁剪坐标计算、快捷键绑定
...
**交付物：**
- [x] GPUI 主题系统化：将 `Palette` 扩展为亮色/暗色两套变量，通过 `Theme` trait 切换
- [x] 设置面板主题选择：新增「主题」下拉框（跟随系统 / 亮色 / 暗色），持久化到 config.toml
- [x] 暗色主题设计：所有组件（侧栏、聊天气泡、设置面板、传输卡片、文件接收对话框）适配暗色配色
- [x] 跟随系统主题：检测操作系统深色模式偏好（Windows 注册表 / macOS NSUserDefaults），自动切换
- [x] 冒烟测试：亮色 / 暗色主题切换后所有界面元素可读、对比度合规
...
**交付物：**
- [x] Blowfish 算法实现（或引入 `blowfish` crate）：ECB/CBC 模式，密钥长度 4-56 字节
- [x] 密钥派生：基于 MAC 地址 + 用户名的密钥生成算法（飞秋规范）
- [x] ENCOPT 编码/解码：`IPMSG_ENCOPT (0x00400000)` 标志位的报文加密与解密
- [x] 加密群聊：GroupMsg 加密模式支持，群成员共享密钥
- [x] 协议兼容性测试：与飞秋原版客户端互发加密消息验证互通
- [x] 单元测试：Blowfish 加解密向量测试、密钥派生已知值验证
...
**交付物：**
- [x] 头像存储：`peers` 表新增 `avatar_path` 列，本地缓存头像图片
- [x] 头像设置 UI：设置面板中选择头像图片（裁剪为正方形），保存到本地
- [x] 头像传输协议：设计并实现 FeiQ 非标准扩展报文（头像请求 + 头像响应，走 TCP 下载）
- [x] 侧栏头像显示：peer_row 用头像图片替换当前状态圆点（保留状态指示器叠加）
- [x] 聊天面板头像：消息气泡旁显示发送者头像
- [x] 默认头像：无头像时根据用户名首字母/颜色生成占位头像
